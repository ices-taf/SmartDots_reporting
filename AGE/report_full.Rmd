...
for(i in 1:nfig)
{
  if(i==1){
    ...
    set.caption('**Table X:** Average Percentage Error (APE) table represents the APE per
             modal age and reader, the APE of all readers
...
# AVERAGE PERCENTAGE ERROR TABLE - All readers ###############################
...
# NUMBER OF AGE READINGS PER STOCK - All readers #########################
# Table caption
set.caption('**Table X:** Number of age readings per strata and modal age for all readers combined.')
...
# Table caption
set.caption('**Table X:** CV per strata and modal age for all readers combined.')
...
# Table caption
set.caption('**Table X:** Percentage Agreement per strata and modal age for all readers combined.')
...
# Table caption
set.caption('**Table X:** Average Percentage Error per strata and modal age for all readers combined.')
...
# Table caption
set.caption('**Table X:** Relative Bias per strata and modal age for all readers combined.')
...
<!-- Total number of samples (NSample), number (CountMM) and percentage of multimodal cases (PercMM) text -->
```{r summary_multiple_modes_adv}
summary_multiple_modes_adv <-
    ...
    PercMM_negexp_weight=unique(ifelse(multimode_cases_tab_negexp_adv$NModes_negexp=="zero", paste(0,"%"), paste(round((nrow(multimode_cases_tab_negexp_adv)/length(unique(ad_long_adv$SampleID)))*100, digits=0),"%"))),
...
# Output table
pander(style_table0(summary_multiple_modes_adv), missing="")
...
<!-- Total number of samples (NSample), number (CountMM) and percentage of multimodal cases (PercMM), PA, CV and APE text -->
```{r summary_statistics_adv}
summary_statistics_adv <-
    c(NSample  = length(unique(ad_long_adv$SampleID)),
    CV  = cv_tab_adv[nrow(cv_tab_adv), "all"],
    PA  = pa_tab_adv[nrow(pa_tab_adv), "total"],
    APE = ape_tab_adv[nrow(ape_tab_adv), "all"])

# Table caption
set.caption('**Table X:** Summary of statistics; Total number of samples (NSample), coefficient of variance (CV), percentage of agreement (PA) and average percentage error (APE) for avanced ages and readers')

# Output table
pander(style_table0(summary_statistics_adv), missing="")

```


In this exchange event, `r summary_statistics_adv[1]` otolith fish individuals were aged. Of those, `r summary_multiple_modes_adv[2]`when the traditional approach (all readers equally weighted) is used to define the mode. The percentage of multiple mode cases is reduced to `r summary_multiple_modes_adv[5]`. The complete list of cases with multiple modes is presented in table X in the annex 3 section, where the ageing from each of the readers participating in the exchange event is presented. When all the otolith samples are considered (both single and multimodal cases) the weighted average percentage agreement based on modal ages for all readers is `r summary_statistics_adv[3]`, with the weighted average CV of `r summary_statistics_adv[2]` and APE of `r summary_statistics_adv[4]`.
...
for(i in 1:nfig)
{
  if(i==1){
    ...
    set.caption('**Table X:** Coefficient of Variation (CV) table presents the
             CV per modal age and advanced reader, the CV of advanced
...
for(i in 1:nfig)
{
  if(i==1){
    # Table caption
    set.caption('**Table X:** Percentage agreement (PA) table represents the PA per
             modal age and reader, advanced the PA of advanced readers
...
for(i in 1:nfig)
{
  if(i==1){
    # Table caption
    set.caption('**Table X:** Average Percentage Error (APE) table represents the APE per
             modal age and reader, the APE of advanced readers
             combined per modal age and a weighted mean of the APE per reader.')
    selec=data[,c(1:basicols, (i+basicols+((N-1)*(i-1))):((i+basicols+((N-1)*(i-1)))+(N-1)))]
    pander(style_table1(selec), missing = "-")
  } else { if(i<nfig){
...
for(i in 1:nfig)
{
  if(i==1){
    # Table caption
    set.caption('**Table X:** The relative bias (as the difference between the mean and modal age) per
             modal age and advanced reader is presented, as well as the weighted mean relative bias per reader and the relative bias per modal age for 
...
# Output table
plot_bias_all(ad_long_adv, sel_readers="Advanced readers")
...
# Output table
plot_growth(dist, ad_long_adv, stratif="no_stratification")
...
```{r results_by_strata_and_reader_title_adv, eval = print_strata()}
stratum <- params$strata[istrata]
# Section title
asis_output(paste0("**Comparison results by reader on strata: ", stratum, "**"))
```
...
<!-- Select data by strata -->
```{r select_data_by_strata_adv, eval = print_strata()}

ad_long_adv_by_strata=ad_long_adv[ad_long_adv[["strata"]]==stratum,]
...
<!-- Total number of samples (NSample), number (CountMM) and percentage of multimodal cases (PercMM) text -->
```{r summary_multiple_modes_adv_by_strata, eval = print_strata()}
summary_multiple_modes_adv <-
    c(NSample  = length(unique(ad_long_adv_by_strata$SampleID)),
    PercMM_traditional=unique(ifelse(eval(parse(text=paste0(vsname("multimode_cases_tab_traditional"),"$NModes_trad")))=="zero", paste(0,"%"), paste(round((nrow(eval(parse(text=vsname("multimode_cases_tab_traditional"))))/length(unique(ad_long_adv_by_strata$SampleID)))*100, digits=0),"%"))),
    PercMM_linear_weight=unique(ifelse(eval(parse(text=paste0(vsname("multimode_cases_tab_linear"),"$NModes_linear")))=="zero", paste(0,"%"), paste(round((nrow(eval(parse(text=vsname("multimode_cases_tab_linear"))))/length(unique(ad_long_adv$SampleID)))*100, digits=0),"%"))),
    PercMM_negexp_weight=unique(ifelse(eval(parse(text=paste0(vsname("multimode_cases_tab_negexp"),"$NModes_negexp")))=="zero", paste(0,"%"), paste(round((nrow(eval(parse(text=vsname("multimode_cases_tab_negexp"))))/length(unique(ad_long_adv$SampleID)))*100, digits=0),"%"))),
    PercMM_multistage=unique(ifelse(eval(parse(text=paste0(vsname("multimode_cases_tab_multistage"),"$NModes_multistage")))=="zero", paste(0,"%"), paste(round((nrow(eval(parse(text=vsname("multimode_cases_tab_multistage"))))/length(unique(ad_long_adv$SampleID)))*100, digits=0),"%"))))
...
<!-- Total number of samples (NSample), number (CountMM) and percentage of multimodal cases (PercMM), PA, CV and APE text -->
```{r summary_statistics_adv_by_strata, eval = print_strata()}
summary_statistics_adv <-
    c(NSample  = length(unique(ad_long_adv_by_strata$SampleID)),
    CV  = eval(parse(text=vsname("cv_tab")))[nrow(eval(parse(text=vsname("cv_tab")))), "all"],
    PA  = eval(parse(text=vsname("pa_tab")))[nrow(eval(parse(text=vsname("pa_tab")))), "total"],
...
asis_output(paste0("In this exchange event ", summary_statistics_adv[1], " otolith fish individuals were aged. Of those, ", summary_multiple_modes_adv[2], "when the traditional approach (all readers equally weighted) is used to define the mode. The percentage of multiple mode cases is reduced to ", summary_multiple_modes_adv[5], ". The complete list of cases with multiple modes is presented in table X in the annex 3 section, where the ageing from each of the readers participating in the exchange event is presented. When all the otolith samples are considered (both single and multimodal cases) the weighted average percentage agreement based on modal ages for all readers is ", summary_statistics_adv[3], ", with the weighted average CV of ", summary_statistics_adv[2], " and APE of ",  summary_statistics_adv[4], "."))
...
```{r cv_table_by_strata_adv, eval = print_strata()}
...
for(i in 1:nfig)
{
  if(i==1){
    # Table caption
    set.caption(paste('**Table X:** Strata', stratum, '.Coefficient of Variation (CV) table presents the CV
             per modal age and reader, the CV of advanced readers combined per modal
             age and a weighted mean of the CV per reader'))
    selec=data[,c(1:basicols, (i+basicols+((N-1)*(i-1))):((i+basicols+((N-1)*(i-1)))+(N-1)))]
    pander(style_table1(selec), missing = "-")
  } else { if(i<nfig){
...
asis_output("The percentage agreement per advanced reader per modal age tells how large part of the
readings that are equal to the modal age. The weighted mean including at the
bottom of the table is weighted according to number of age readings.")
...
```{r percentage_agreement_by_strata_adv, eval = print_strata()}
...
for(i in 1:nfig)
{
  if(i==1){
    # Table caption
    set.caption(paste('**Table X:** Strata', stratum, '.Percentage agreement (PA) table represents the PA per
             modal age and advanced reader, the PA of all advanced readers combined per modal
             age and a weighted mean of the PA per reader.'))
    selec=data[,c(1:basicols, (i+basicols+((N-1)*(i-1))):((i+basicols+((N-1)*(i-1)))+(N-1)))]
    pander(style_table1(selec), missing = "-")
  } else { if(i<nfig){
...
```{r average_percentage_error_adv_by_strata, eval = print_strata()}
...
# <!-- Relative bias text -->
asis_output("The relative bias is the difference between the mean age (per modal age per
advanced reader) and modal age. As for the previous tables, a combined bias for advanced
readers and weighted means are calculated.")

```

```{r rb_adv_by_strata, eval = print_strata()}
...
```{r bias_plots_adv_by_strata, fig.width = 5, fig.height = 4, fig.cap = cap_in, eval = print_strata()}
...
# Output figure
plot_bias_all(ad_long_adv_by_strata, sel_readers="Advanced readers")
```
...
```{r bias_table_by_strata_adv, eval = print_strata()}
...
# Output table
pander(style_table3(bias_tab_adv), missing = ".", style = "simple")
...
```{r growth_analysis_by_strata_adv, fig.width = 8, fig.height = 4, fig.cap = cap_in, eval = print_strata()}
...
plot_growth(dist, ad_long_adv_by_strata, stratif=stratum)
```
...
<!-- Select data by strata -->
```{r ref.label= 'select_data_by_strata_adv', eval = print_strata()}
...
<!-- Total number of samples (NSample), number (CountMM) and percentage of multimodal cases (PercMM) text -->
```{r ref.label= 'summary_multiple_modes_adv_by_strata', eval = print_strata()}
...
<!-- Total number of samples (NSample), number (CountMM) and percentage of multimodal cases (PercMM), PA, CV and APE text -->
```{r ref.label= 'summary_statistics_adv_by_strata', eval = print_strata()}
...
```{r ref.label= 'cv_table_by_strata_adv', eval = print_strata()}
...
```{r ref.label= 'percentage_agreement_by_strata_adv', eval = print_strata()}
...
```{r ref.label= 'average_percentage_error_adv_by_strata', eval = print_strata()}
...
```{r ref.label= 'rb_adv_by_strata', eval = print_strata()}


```

```{r ref.label= 'bias_plots_adv_by_strata', fig.width = 5, fig.height = 4, fig.cap = cap_in, eval = print_strata()}
...
```{r ref.label= 'bias_table_by_strata_adv', eval = print_strata()}
...
```{r ref.label= 'growth_analysis_by_strata_adv', fig.width = 8, fig.height = 4, fig.cap = cap_in, eval = print_strata()}
...
# Table caption
set.caption('**Table X:** Number of age readings per strata and modal age for advanced readers combined.')
...
# Table caption
set.caption('**Table X:** CV per strata and modal age for advanced readers combined.')
...
# Table caption
set.caption('**Table X:** Percentage Agreement per strata and modal age for advanced readers combined.')
...
# Table caption
set.caption('**Table X:** Average Percentage Error per strata and modal age for advanced readers combined.')
...
# Table caption
set.caption('**Table X:** Relative Bias per strata and modal age for advanced readers combined.')
...
# Table caption
cap_in <- '**Figure X:** Plot of average distance from the centre to the winter
           rings for advanced readers by strata. The boxes represent 
           the median, upper and lower box boundaries of the interquartile range,
           whiskers represent the minimum and maximum values and the dots represent 
           the outliers.'
